...
from release.backend.diff import (
    apply_delta,
    apply_deltas,
    diff_to_delta,
    invert_delta,
...
def test_offsets_are_utf16_code_units_not_code_points():
    ...
    assert delta[0] == {"retain": 2}


def test_astral_characters_survive_a_chain_of_edits():
    revisions = ["", f"a{EMOJI}b", f"a{EMOJI}{EMOJI}b", f"{EMOJI}b", ""]
    deltas = [diff_to_delta(before, after) for before, after in zip(revisions, revisions[1:])]
