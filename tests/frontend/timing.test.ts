...
describe("what a transaction carries about when it happened", () => {
  it("stamps the client's zone on every kind of request", async () => {
    ...
    await workspace.create("a.py", "print()");
    await workspace.folder("src");
    await workspace.write("a.py", "print(1)");
    await workspace.move("a.py", "b.py");
    await workspace.remove("b.py");
    workspace.stop();
...
describe("the mtime a client shows before the server has answered", () => {
  ...
  it("moves to the queued transaction, in the zone it was made in", () => {
    ...
    const shown = effective.of(confirmed.snapshot([file]), items.entries()).get(file.id)!;
  ...
  it("snaps back to the confirmed mtime when the work is refused", () => {
    ...
    expect(effective.of(map, items.entries()).get(file.id)!.modified.accepted).toBeNull();
    items.evict([request.transaction]);
    expect(effective.of(map, items.entries()).get(file.id)!.modified).toEqual(
      file.modified,
    );
  });
  ...
  it("gives a queued create an mtime of its own", () => {
    ...
    const shown = effective.of(confirmed.empty(), items.entries()).get(id)!;
